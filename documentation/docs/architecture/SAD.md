...
Reports are saved to the folder configured by `Reporting:Folder` (default: `reports/`) as `{session-name}.report.md` and include event distribution, utterance analysis, intent detection results, latency statistics, and log insights.
